...
**annotation_colours:** by default, the heatmap package (the plotly-powered iheatmapr) will automatically assign colours for the annotations. Alternatively, an RColorBrewer palette or a list of colours (e.g. hex values) can be assigned to individual variables (not every variable needs to be assigned custom colours).
**annotation_range:** for numeric variables that share an underlying range of values, but the data varies, it is possible to predefine the range to share the same colour scale. For each annotation variable, a minimum and maximum value can be provided through a list. A common use case is when the range of values of a clinical variable drop off at a later time point.
