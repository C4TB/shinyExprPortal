...
The expected format for count matrices contains sample IDs in columns and gene symbols in rows (Ensembl IDs would also work). The **best** format for the portal is saving an R object of matrix type as an .rds file, although it is also possible to save the matrix as a CSV or TSV file. If using the latter, ensure that the first row (with exception of the first column) contain the sample IDs, and that the first column contain the transcript identifiers.
...
It is also possible to use the [{fst}](https://cran.r-project.org/web/packages/fst/index.html) and [{qs}](https://cran.r-project.org/web/packages/qs/index.html) packages for compressed data files.

**Measures table**

The measures table follows the format of one row per subject (that may have one or more samples collected) with the measures across columns. A data.frame or tibble object can be saved in an .rds file and used, otherwise CSV or TSV files are also supported.
...
For datasets where a subject has more than one sample (e.g. samples over time, from different tissues or combinations thereof), a lookup table should be constructed and save as a data.frame or tibble format in .rds, or CSV or TSV. 

This table will map subject identifiers to sample IDs, with the expression matrix containing counts for *all* samples in the dataset. The table should also contain metadata that allows selection of groups of samples based on how they vary. For example, if subjects and samples vary on time, drug groups and tissues, the lookup table will require one column for each category. The following is an example of such a table:
...
table <- data %>%
  ...
  transmute(list = paste(gene, collapse = ","))
```
:::
