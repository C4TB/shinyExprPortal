...
mod_geneModulesCorr_ui <- function(module_name, config, module_config) {
  geneModulesCorr_tab(
    ...
    sources_names = lapply(
      module_config$modules_data,
      function(x) x$name
    ),
      ...
      verticalLayout(
        ...
        wellPanel(
          selectizeInput(
            ...
            label =
              with_red_star("Select a clinical variable to compare across"),
...
mod_geneModulesCorr_server <- function(module_name, config, module_config) {
  moduleServer(module_name, function(input, output, session) {
    ...
    selected_module_profile <- reactive({
      ...
      computeModuleProfile(
        module_medians, input$selected_module,
    ...
    output$profile_plot <- renderPlot({
      ...
      genes <-
        modules_data[
          which(modules_data$modules == input$selected_module),
          "genes"
        ]
    ...
    observeEvent(c(input$selected_clinical, selected_module_profile()), {
      ...
      plotWidth <- {
        if (ac_length < 4) ac_length * 200 else 800
      }
