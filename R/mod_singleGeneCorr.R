    ...
    observe({
      ...
      for (i in seq_along(tab_output_list)) {
        local({
          ...
          if (output_type == "cat") {
            for(col in colnames(tab_measures))
              data.table::set(tab_measures, j = col, value = as.character(tab_measures[[col]]))
          }
          corr_df_subset <- data.frame()
          if (!is.na(corr_df))
